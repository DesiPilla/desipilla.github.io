...
[![A demo of how to use the Survey Analyzer app and read its results](img/title_slide.png)](survey-analyzer-demo.mp4?raw=true)
...
![How to save a Google Sheets document (left) or CSV file (right) as an Excel Workbook](img/file_type.png)
...
![Formatting input files with all questions on a single worksheet.](img/format_style_1.png)
...
![Formatting input files with each questions on a separate worksheet.](img/format_style_2.png)
...
![Navigate to the ViacomCBS workspace](img/find_viacomcbs_workspace.png)
 
In the *Search* bar at the top of the window, type in Survey Analyzer and select the App.
 
 ![ ](img/find_survey_analyzer.png)

 ![ ](img/survey_analyzer_beginning.png)
...
![Begin the analysis by saying "Analyze my file" and following the prompts](img/starting_analysis.png)
...
![Each question will have two corresponding result sheets.](img/sheet_names.png)
...
![Top topics table](img/top_topics.png)
...
![Topic dominance table](img/topic_dominance.png)
...
![The HTML files contain an interactive bubble chart that displays the learned topics.](img/visualization.png)
...
![The HTML files also contain a dynamic table that contains the top 20 responses for each learned topic.](img/html_table.png)
